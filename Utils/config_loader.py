...
def check_param(param_name: str, obj: dict | configparser.SectionProxy,
    ...
    if value is None:
        if raise_ex_if_not_exists:
            raise Exception  # todo: создать кастомное исключение: отсутсвует параметр.
        return value

    value = value.strip()
    if not value:
        raise Exception  # todo: создать кастомное исключение: параметр пуст.

    if valid_values is not None:
        if value not in valid_values:
            raise Exception  # todo: создать кастомное исключение: значение параметра не в списке допустимых.
...
def load_lots_config(config_path: str) -> configparser.ConfigParser:
    ...
    for alias in config.sections():
        check_param("name", config[alias])
        check_param("exclude_auto_restore", config[alias], valid_values=["0", "1"], raise_ex_if_not_exists=False)
    return config
...
def load_auto_delivery_config(config_path: str) -> configparser.ConfigParser:
    ...
    for lot in config.sections():
        ...
        lot_response = check_param("response", config[lot])

        # Проверяем указан параметр products_storage_type и если да, то правильно ли.
        products_storage_type = check_param("products_storage_type", config[lot], raise_ex_if_not_exists=False)
        ...
        if "$product" not in lot_response:
            raise Exception  # todo: создать и добавить кастомное исключение: отсутствует переменная в тексте.

        # Проверяем указан ли параметр products_file_name и существует ли файл, указанный в этом параметре.
        products_file_path = check_param("products_file_path", config[lot])
        if not os.path.exists(products_file_path):
            raise Exception  # todo: создать и добавить кастомное исключение: файл с товарами отсутствует.
        ...
        if products_storage_type == "json":
            ...
            try:
                ...
                raise Exception  # todo: создать и добавить кастомное исключение: невалидный формат данных.
            # Проверяем наличие товара
            if len(products) < 1:
                raise Exception  # todo: создать и добавить кастомное исключение: кол-во товара = 0
            ...
            db_table_name = check_param("db_table_name", config[lot])
            check = check_db_table_exists(products_file_path, db_table_name)
            if not check:
                raise Exception  # todo: создать и добавить кастомное исключение: таблицы не существует.
            ...
            db_column_name = check_param("db_column_name", config[lot])
            check = check_table_column_exists(products_file_path, db_table_name, db_column_name)
            if not check:
                raise Exception  # todo: создать и добавить кастомное исключение: столбца не существует.
